    ...
    , IEndDragHandler
{
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        {
            if (CharlestonBox.parent.gameObject.activeSelf)
    ...
    public void DoubleClickCharleston()
    {
        ...
        {
            for (int i = 0; i < 3; i++) // check for an empty spot and move there
            {
                Transform chSpot = CharlestonBox.GetChild(i);
                if (chSpot.childCount == 0)
                {
                    MoveTile(chSpot);
                    CClient.ClientPassArr[i] = TileID;
                    break;
                }
            }

            if (transform.IsChildOf(RackPrivateTF)) // if there were no empty spots
            {
                CharlestonBox.GetChild(2)
                               .GetComponentInChildren<TileLocomotion>()
                               .MoveTile(RackPrivateTF);
                MoveTile(CharlestonBox.GetChild(2));
                CClient.ClientPassArr[2] = TileID;
            }
        }
        else {
            int chSpotId = transform.parent.GetSiblingIndex();
            MoveTile(RackPrivateTF);
            CClient.ClientPassArr[chSpotId] = -1;     
        }
        CClient.CheckReadyToPass();
    }
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        if (transform.parent.IsChildOf(CharlestonBox)) // TODO: test this and other PassArr updates
        {
            int chSpotId = transform.parent.GetSiblingIndex();
            CClient.ClientPassArr[chSpotId] = -1;
        }
        transform.SetParent(Refs.Dragging); // Keep the face in a spot lower on the
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        // drop on charleston
        else if (raycastTFs.Contains(CharlestonBox)
            && eventData.pointerCurrentRaycast.gameObject != CharlestonBox.gameObject)
    ...
    void DropOnCharleston(PointerEventData eventData)
    {
        ...
        CClient.ClientPassArr[charlestonSpotTF.GetSiblingIndex()] = TileID;
    }
